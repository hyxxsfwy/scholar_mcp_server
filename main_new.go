...
func searchScholarPapers(ctx context.Context, req *mcp.CallToolRequest, params *ScholarSearchParam) (*mcp.CallToolResult, any, error) {
	...
	if os.Getenv("SCOPUS_API_KEY") != "" {
		config.EnableScopus = true
		config.ScopusAPIKey = os.Getenv("SCOPUS_API_KEY")
	}
	if os.Getenv("ADSABS_API_KEY") != "" {
		config.EnableAdsabs = true
		config.AdsabsAPIKey = os.Getenv("ADSABS_API_KEY")
	}

	// 如果指定了特定数据源，则只启用这些数据源
	if len(params.EnabledSources) > 0 {
		config.EnableArxiv = false
		config.EnableSemanticScholar = false
		config.EnableCrossref = false
		config.EnableScopus = false
		config.EnableAdsabs = false
		config.EnableSciHub = false

		for _, source := range params.EnabledSources {
			switch source {
			case "arxiv":
				config.EnableArxiv = true
			case "semantic_scholar":
				config.EnableSemanticScholar = true
			case "crossref":
	...
	// 创建聚合器并搜索
	agg := aggregator.NewScholarAggregator(config)
...
func main() {
	...
	log.Printf("[INFO] 正在初始化聚合式学术论文检索MCP服务器...")
	...
	log.Printf("[INFO]   - SCOPUS_API_KEY: %s", scopusStatus)
	log.Printf("[INFO]   - ADSABS_API_KEY: %s", adsabsStatus)
	log.Printf("[INFO] ")
	...
	log.Printf("[INFO]   指定数据源: {\"query\": \"quantum computing\", \"enabled_sources\": [\"arxiv\", \"semantic_scholar\"]}")
